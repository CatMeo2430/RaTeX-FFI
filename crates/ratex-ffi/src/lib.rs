...
use ratex_render::{render_to_rgba, RenderOptions};
...
pub unsafe extern "C" fn ratex_render_bitmap(
    ...
    let render_options = match parse_render_options(opts) {
        Ok(options) => options,
        Err(msg) => return err_result(&msg),
    };
    ...
    match do_render_bitmap(latex_str, style, layout_color, &render_options) {
